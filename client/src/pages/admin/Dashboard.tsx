        ...
        {summaryStats.map((stat, index) => (
          ...
          >
            <div className="flex items-start gap-4">
              <div
                className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 ${stat.bg} ${stat.color}`}
              >
                <stat.icon size={24} />
              </div>
              <div>
          ...
          <div className="xl:hidden flex flex-col divide-y divide-border-muted">
            {recentJobs.length > 0 ? (
              recentJobs.map((job, idx) => (
                <div
                  key={idx}
                  className="p-4 flex flex-col gap-3 hover:bg-bg-light transition-colors"
                >
              ...
              <tbody className="divide-y divide-border-muted">
                {recentJobs.length > 0
                  ? recentJobs.map((job, idx) => (
                      <tr
                        key={idx}
                        className="hover:bg-bg-light transition-colors text-fg-muted"
                      >
        ...
        <div className="xl:col-span-1 bg-bg-base border border-border-muted rounded-xl shadow-sm flex flex-col">
          ...
          <div className="p-4 xl:p-5 space-y-4 overflow-y-auto max-h-100">
            {techActivity.map((tech, index) => (
              <div
                key={index}
                className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-border-muted last:border-0 last:pb-0"
              >
